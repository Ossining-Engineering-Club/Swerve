...
void Drivetrain::FieldRelativeKinematics(units::velocity::meters_per_second_t xspeed, units::velocity::meters_per_second_t yspeed,
                                        units::radians_per_second_t angularVelocity,units::angle::radian_t CurrentAngle)
{
    //For Linear Motion
    
    /*gives Chassis object a value of the desired conditions so they could be later evaluated via inverse  kinematics to attain the wanted states of the modules and return the angle and speed of each wheel in a matrix*/
...
void Drivetrain::RobotRelativeKinematics(units::meters_per_second_t  xspeed, units::meters_per_second_t  yspeed, 
